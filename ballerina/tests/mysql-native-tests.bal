...
function mysqlNativeExecuteTestNegative2() returns error? {
    ...
    if executionResult is persist:Error {
        test:assertTrue(executionResult.message().includes("Table 'test.Departments' doesn't exist."));
    } else {
        test:assertFail("persist:Error expected.");
    }
...
function mysqlNativeQueryTestNegative() returns error? {
    ...
    if departments is persist:Error {
        test:assertTrue(departments.message().includes("Table 'test.Departments' doesn't exist."));
    } else {
        test:assertFail("persist:Error expected.");
    }
