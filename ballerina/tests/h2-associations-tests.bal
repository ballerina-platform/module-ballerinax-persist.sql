...
@test:Config {
    ...
    dependsOn: [h2EmployeeDeleteTestNegative]
}
function h2EmployeeRelationsTest() returns error? {
...
function h2DepartmentRelationsTest() returns error? {
    H2RainierClient rainierClient = check new ();
...
@test:Config {
    groups: ["associations", "h2"],
    dependsOn: [h2EmployeeRelationsTest]
}
function h2WorkspaceRelationsTest() returns error? {
    H2RainierClient rainierClient = check new ();
...
function h2BuildingRelationsTest() returns error? {
    H2RainierClient rainierClient = check new ();
