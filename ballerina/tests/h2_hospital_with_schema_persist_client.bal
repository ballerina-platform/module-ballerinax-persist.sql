...
public isolated client class H2HospitalWithSchemaClient {
    *persist:AbstractPersistClient;
    ...
    private final record {|SQLMetadata...;|} metadata = {
        ...
        [PATIENT]: {
            ...
            keyFields: ["id"],
            joinMetadata: {appointments: {entity: Appointment, fieldName: "appointments", refSchema: "hospital", refTable: "appointment", refColumns: ["patient_id"], joinColumns: ["IDP"], 'type: MANY_TO_ONE}}
        },
        [DOCTOR]: {
            entityName: "Doctor",
            tableName: "Doctor",
            ...
            keyFields: ["id"],
            joinMetadata: {appointments: {entity: Appointment, fieldName: "appointments", refSchema: "hospital", refTable: "appointment", refColumns: ["doctorId"], joinColumns: ["id"], 'type: MANY_TO_ONE}}
        }
    };

    public isolated function init() returns persist:Error? {
        jdbc:Client|error dbClient = new (url = h2WithSchema.url, user = h2WithSchema.user, password = h2WithSchema.password, options = {...h2WithSchema.connectionOptions});
        if dbClient is error {
            return <persist:Error>error(dbClient.message());
        }
        self.dbClient = dbClient;
        // Update the metadata with the schema name
        if h2WithSchema.defaultSchema != () {
            lock {
                foreach string key in self.metadata.keys() {
                    SQLMetadata metadata = self.metadata.get(key);
                    if metadata.schemaName == () {
                        metadata.schemaName = h2WithSchema.defaultSchema;
                    }
                    ...
                    foreach [string, JoinMetadata][_, joinMetadata] in joinMetadataMap.entries() {
                        if joinMetadata.refSchema == () {
                            joinMetadata.refSchema = h2WithSchema.defaultSchema;
                        }
                    }
                }
            }
