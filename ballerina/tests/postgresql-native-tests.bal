...
@test:Config {
    ...
    dependsOn: [postgresqlEmployeeRelationsTest, postgresqlWorkspaceRelationsTest, postgresqlBuildingRelationsTest, postgresqlDepartmentRelationsTest]
}
function postgresqlNativeExecuteTest() returns error? {
...
@test:Config {
    groups: ["native", "postgresql"],
    dependsOn: [postgresqlNativeExecuteTest]
}
function postgresqlNativeExecuteTestNegative1() returns error? {
    PostgreSQLRainierClient rainierClient = check new ();
    ...
    if executionResult is persist:Error {
        test:assertTrue(executionResult.message().includes("duplicate key value violates unique constraint \"department_pkey\""));
    } else {
...
function postgresqlNativeExecuteTestNegative2() returns error? {
    PostgreSQLRainierClient rainierClient = check new ();
    ExecutionResult|persist:Error executionResult = rainierClient->executeNativeSQL(`
    ...
    if executionResult is persist:Error {
        test:assertTrue(executionResult.message().includes("relation \"departments\" does not exist"));
    } else {
        test:assertFail("persist:Error expected.");
    }
...
function postgresqlNativeQueryTest() returns error? {
    PostgreSQLRainierClient rainierClient = check new ();
...
function postgresqlNativeQueryTestNegative() returns error? {
    PostgreSQLRainierClient rainierClient = check new ();
    ...
    if departments is persist:Error {
        test:assertTrue(departments.message().includes("relation \"departments\" does not exist"));
    } else {
        test:assertFail("persist:Error expected.");
    }
...
function postgresqlNativeQueryComplexTest() returns error? {
    PostgreSQLRainierClient rainierClient = check new ();
...
function postgresqlNativeTransactionTest2() returns error? {
    PostgreSQLRainierClient rainierClient = check new ();
...
@test:Config {
    groups: ["postgresql", "native"],
    dependsOn: [postgresqlAllTypesDeleteTest]
}
function postgresqlNativeAllTypesTest() returns error? {
    PostgreSQLRainierClient rainierClient = check new ();
