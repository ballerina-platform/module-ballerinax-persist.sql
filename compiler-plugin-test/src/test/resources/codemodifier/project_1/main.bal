...
public function main() returns error? {
    ...
    int[] names = check from entities:Product student in mcClient->/products(targetType = entities:Product)
    ...
    entities:Product[] products = check from var e in mcClient->/products.get(targetType = entities:Product)
    ...
    entities:Product[] products2 = check from var e in mcClient->/products(targetType = entities:Product)
            where e.id == value || e.id == 6 && e.id % 2 == 0
            select e;

    entities:Product[] products3 = check from var e in mcClient->/products(targetType = entities:Product)
            where e.id == value || e.id == 3 && e.id >= 2
            select e;

    entities:Product[]|error result1 = from var e in mcClient->/products(targetType = entities:Product)
                where e.id == value && e.id <= 5
                select e;

    entities:Product[] products4 = check from var e in mcClient->/products(targetType = entities:Product)
                where (e.id == value || e.id == 5) && e.id != 6
                select e;

    entities:Product[] results1 = check from entities:Product e in mcClient->/products(targetType = entities:Product)
